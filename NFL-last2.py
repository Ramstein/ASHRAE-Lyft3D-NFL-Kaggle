...
def create_features(train, deploy=False):
    ...
    def update_orientation(df, yardline):
        df['X_up'] = df[['X', 'PlayDirection']].apply(lambda x: new_X(x[0], x[1]), axis=1)
        df['Orientation_up'] = df[['Orientation', 'PlayDirection']].apply(lambda x: new_orientation(x[0], x[1]),
                                                                          axis=1)
        df['Dir_up'] = df[['Dir', 'PlayDirection']].apply(lambda x: new_orientation(x[0], x[1]), axis=1)
    ...
    def combine_features(relative_to_back, defense, static, deploy=deploy):
        ...
        feats = ['GameId', 'PlayId', 'Team', 'X', 'Y', 'S', 'A', 'Dis', 'Orientation', 'Dir',
                 'NflId', 'JerseyNumber', 'Season', 'Quarter',
                 ...
                 'WindDirection', 'X_up', 'Orientation_up', 'Dir_up', 'YardLine', 'TimeDelta',
                 'GameClock_sec', 'GameClock_minute', 'PlayerAge', 'OffenseDB', 'OffenseDL',
                 'OffenseLB', 'OffenseOL', 'OffenseQB', 'OffenseRB', 'OffenseTE', 'OffenseWR',
                 'DefenseDB', 'DefenseDL', 'DefenseLB', 'DefenseOL', 'Orientation_sin',
                 'Orientation_cos', 'Dir_sin', 'Dir_cos', 'diffScoreBeforePlay', 'IsRusher',
                 'RusherTeam', 'IsRusherTeam']
        if deploy: feats.remove('Yards')
        static = static[static['NflId'] == static['NflIdRusher']][feats].drop_duplicates()
    ...
    df = update_orientation(train, yardline)
    back_feats = back_features(df)
    rel_back = features_relative_to_back(df, back_feats)
    def_feats = defense_features(df)
    static_feats = static_features(df)
    basetable = combine_features(rel_back, def_feats, static_feats, deploy=False)
    return basetable
