...
#
#
# # train = pd.read_csv(r'C:\Datasets\nfl-big-data-bowl-2020\train.csv', dtype={'WindSpeed': 'object'})
# # # train = train[:2200]
# # print(train.shape)
# # print(train.head())
#
...
# #     train["PlayerHeight"] = train["PlayerHeight"].str.split("-", expand=True)[0].astype(float) * 30.48 + \
# #                          train["PlayerHeight"].str.split("-", expand=True)[1].astype(float) * 2.54  # in centimeters
# #
# #
# #     ## Time
...
#
#
# # print('-----pickling the preprocessed training data')
# # pd.to_pickle(train, "train.pkl")
#
# print('laoding the train.pkl file.')
# with open('NFL_1_train.pkl', 'rb') as f:
#     train = pickle.load(f)
#
#
# def drop(train):
...
#         (int(emb_cat_players.shape[1]), int(emb_cat_players.shape[2]) * int(emb_cat_players.shape[3])))(emb_cat_players)
#
...
#                   optimizer=keras.optimizers.Adam(lr=0.002, decay=1e-4),
#                   metrics=[])
#
#     # checkpoint
#     path_checkpoint = "NFL_1_-{epoch:02d}-{val_out_soft_loss:.4f}.h5"
#     callbacks_list = [
#         EarlyStopping(monitor='accuracy', min_delta=0.001, patience=5, verbose=0, mode='auto', baseline=None,
#                       restore_best_weights=True),
#         ModelCheckpoint(path_checkpoint, monitor='val_out_soft_loss',
...
# #         losses.append(loss)
#
# model, callbacks_list = get_model()
#
# for k in range(5):
#     kfold = KFold(5, random_state=42 + k, shuffle = True)  # k=1
#     for k_fold, (tr_inds, val_inds) in enumerate():
#         print("-----------")
#         print("-----------")
...
#         # losses.append(loss)
#
# model.save('RSNA_efficientNet_B7_01.h5', overwrite=True)
# print("Saved model to disk: RSNA_efficientNet_B7_01.h5")
#
#
#
# model.fit_generator()
...
#
#
# p = [2,2,2,2,2,26,2,26,16,15,15,165]
#
# y_pred_nn = (np.array(p)).mean()
#
#
#
#
...
# #
# #     ### divide
# #     test_dense_players = [test_dense[dense_player_features].iloc[np.arange(k, len(test), 22)].reset_index(drop=True) for k
# #                           in range(22)]
...
#
#
# from keras.layers import Dense, Flatten, Conv2D, BatchNormalization, GlobalMaxPooling2D, MaxPooling2D
#
...
from keras.layers import BatchNormalization, GaussianNoise, Activation, Conv1D
def get_model():
    model = Sequential()

    model.add(Conv1D(filter=32, kernel_size=2, strides=1, padding='same',activation='softmax'))
    ...
    model.add(Conv2D(filters=512, kernel_size=(2,2), strides=(2,2), padding='same', use_bias=True))
...
for epoch in range(1, epochs + 1):
    ...
    checkpoint_filename = "lyft3d_checkpoint_epoch_{}.pth".format(epoch)
...
odel = get_unet_model(num_output_classes=len(classes)+1)

model.load_state_dict(torch.load(checkpoint_filepath, map_location='cude:2'))

import base64
import pandas as pd
from IPython.display import HTML, FileLink

# def create_download_link( df, title = "Download CSV file", filename = "__notebook_source__.ipynb"):
...
html.format(payload=payload, filename=filename, title = 'Download CSV File.')
HTML(html)


#     return HTML(html, url='hello').url()
#
# # for i in range(3):
# create_download_link(df)

local_file = FileLink('__notebook_source__.ipynb')
# display(local_file)
